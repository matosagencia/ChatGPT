<?php 
	...
	class AdminLinkstorepsimporterController extends ModuleAdminController {
		...
		*/
		...
		public function __construct()
		{
			...
			$this->_filter = "AND id_product IS NULL";
		...
		public function initPageHeaderToolbar() {
			...
			$adminlinkimport = Context::getContext()->link->getAdminLink('AdminLinkstorepsimporter', true, []);
		...
		public function initToolbar() {
			...
			$this->toolbar_title = 'Importaciones de productos';
		}
		...
		public function displayGerarPDFLink($token = null, $id) { 
			...
			if (!array_key_exists('gerarpdf', self::$cache_lang)) { 
				...
				$this->context->smarty->assign(array(
					'id' => $id,
					'urlenvio' => self::$currentIndex.'&'.$this->identifier.'='.$id.'&gerarpdf&token='.($token != null ? $token : $this->token).'&id_category',
					'idd' => $idd,
					'idml' => $idml,
					'imp' => 1,
	...
	$supplierId = SupplierCore::getIdByName($supplierName);
				...
				foreach ($languages as $language) 
				{
					...
					$Product->link_rewrite[$language['id_lang']] = trim(substr(str_replace(".","",str_replace("+","-",str_replace("%","-",$this->removeinvalidos($nomelink)))),0,128));
					//trata a descricao
					...
					$Product->meta_keywords[$language['id_lang']] = trim(str_replace("+",",",substr(urlencode(utf8_encode($linkstoredata[0]['DESCRIPCION'])),0,255)));					
					$Product->description[$language['id_lang']] = pSQL(trim($linkstoredata[0]['DESCRIPCION2']));	
				}
				$Product->id_shop_default = Configuration::get('PS_SHOP_DEFAULT');
				//se possui comissão predefinida
				if(Configuration::get('IMPORT_PRICE_LINKSTORE')){
						if(!empty($linkstoredata[0]['commision']))
							$Product->price =  $this->porcentagem_xn ( $linkstoredata[0]['commision'], $linkstoredata[0]['PRECIOD4']);
						else
				...
				$id_manufacturer = Db::getInstance()->getValue('SELECT id_manufacturer FROM '._DB_PREFIX_.'manufacturer WHERE IDa = "'.pSQL($linkstoredata[0]['IDMANUFACTURE']).'"');
				...
				$Product->Add();
				  //adiciona tag
					$tag_list[] = $linkstoredata[0]['CODIGO'];
					Tag::addTags($language['id_lang'], $Product->id, $tag_list);
				...
				{	
					...
					'quantity' => $linkstoredata[0]['CANTIDAD']), '`id_product` = '.$Product->id);	
					
					Db::getInstance()->update('stock',array(
					'physical_quantity' => $linkstoredata[0]['CANTIDAD'], 'usable_quantity' => 0), '`id_product` = '.$Product->id);	
					
					Db::getInstance()->update('stock_available',array(
					'physical_quantity' => $linkstoredata[0]['CANTIDAD'], 'quantity' => $linkstoredata[0]['CANTIDAD']), '`id_product` = '.$Product->id);		
				}
