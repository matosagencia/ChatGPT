<?php 
	...
	class AdminLinkstorepsupdateController extends ModuleAdminController {
		...
		*/
		...
		public function __construct()
		{
			...
			$this->_filter = " AND a.id_product > '0'";											
		    $this->_join = 'INNER JOIN `' . _DB_PREFIX_ . 'product` p ON (p.`id_product` = a.`id_product`) LEFT JOIN `' . _DB_PREFIX_ . 'product_shop` ps ON (ps.`id_product` = p.`id_product` AND ps.`id_shop` = ' . (int)$this->context->shop->id . ')';

			$this->_select = 'a.`IDa`, a.`CODIGO`, a.`DESCRIPCION`, a.`PRECIOLISTA`, a.`PRECIOD4`, a.`CANTIDAD`, ps.`price` as `price`, a.`commision`, a.`id_product`, a.`id_category`';
		...
		public function initContent() {
			...
			if(Tools::isSubmit('gerarpdf5')){
				$pcoint = Db::getInstance()->ExecuteS('SELECT * FROM `'._DB_PREFIX_.'linkstoreps` WHERE IDa = '.Tools::getValue('IDa').' AND overstock = 0');
			...
			if(Tools::isSubmit('gerarpdf6')){
				...
				if(!empty(Tools::getValue('id_product')))
					header('Location: '.Context::getContext()->link->getAdminLink('AdminProducts', true, ['id_product' => Tools::getValue('id_product')]));
			}
			...
			if(Tools::isSubmit('gerarpdf7')){
				...
				$idp = Db::getInstance()->ExecuteS('SELECT * FROM `'._DB_PREFIX_.'product` WHERE id_product = '.Tools::getValue('id_product').'');
				//verifica se ha codigo equivalente
				$idl_product = Db::getInstance()->ExecuteS('SELECT * FROM `'._DB_PREFIX_.'linkstoreps` WHERE `CODIGO` = "'.trim($idp[0]['reference']).'"');
				//executa veriifcar se ha product ja cadastrado
				if(!empty($idl_product)){
					//verifica se o produto linkstore ja possui cadastro anteior
					if(empty($idl_product[0]['id_product'])){
						//se nao tem gravacao efetua a gravacao
						Db::getInstance()->update('linkstoreps',array('id_product' => Tools::getValue('id_product'),'id_category' => $idp[0]['id_category_default']), '`IDa` = '.$idl_product[0]['IDa']);	
						Db::getInstance()->update('product',array('IDa' => $idl_product[0]['IDa'], 'id_supplier' => $supplierId), '`id_product` = '.Tools::getValue('id_product'));	
						//atualiza o codigo do linkstoreps e da descricao
						$position = Db::getInstance()->ExecuteS('SELECT MAX(position) FROM `'._DB_PREFIX_.'category_product` WHERE id_category ='.$idp[0]['id_category_default']);
						$sqli = array(
					...
					$this->displayWarning('otro producto ya está combinado con este código de linkstore!');
				}
		...
		public function initPageHeaderToolbar() {
			...
			$adminlinkimport = Context::getContext()->link->getAdminLink('AdminLinkstorepsupdate', true, []);
            ...
            'href' => Tools::getShopDomainSsl(true, true).__PS_BASE_URI__.'modules/linkstoreps/extractor/sync.php?adminlink='.$adminlinkimport,
		...
		public function displayGerarPDFLink($token = null, $id) { 
			...
			if (!array_key_exists('gerarpdf', self::$cache_lang)) { 
				...
				'idd' => $idd,
				'urlenvio2' => Tools::getShopDomainSsl(true, true).__PS_BASE_URI__.'modules/linkstoreps/controllers/admin/updatepriceqty.php?adminlink='.$adminlinkim.'&'.$this->identifier.'='.$id.'&update=stock&token='.($token != null ? $token : $this->token).'&adminlink='.$adminlinkimport,
		...
		public function statusmercado($id)
		{
			...
			if(empty($prd[0]['id_product']))
			{
				...
				{	
					...
					$stockid = Db::getInstance()->ExecuteS('SELECT * FROM '._DB_PREFIX_.'stock WHERE `id_product` = '.$linkstoredata[0]['id_product']);
					
				foreach($stockid as $stk){
					
					Db::getInstance()->update('stock',array(
															'physical_quantity' => $linkstoredata[0]['CANTIDAD'], 
															'usable_quantity' => $linkstoredata[0]['CANTIDAD']), 
		...
		public function descmercado($id)
		{
			...
			$linkstoredata = Db::getInstance()->ExecuteS('SELECT * FROM '._DB_PREFIX_.'linkstoreps WHERE IDa = "'. $id .'" AND overstock = 0');
