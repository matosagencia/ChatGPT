<?php
    ...
    if ($ida[0]['CODIGO'] != trim(str_replace(".", "", $thirty_step[2]))) {
        $updateData = array(
            'CODIGO' => trim(str_replace(".", "", $thirty_step[2])),
            'DESCRIPCION' => trim($thirty_step[3]),
            'CANTIDAD' => trim($thirty_step[4]) > 0 ? trim($thirty_step[4]) : 0,
            'TRANSITO' => trim($thirty_step[5]),
            'FECHALLEGADA' => trim($thirty_step[6]),
            'PRECIOLISTA' => trim($thirty_step[7]),
            'PRECIOD4' => trim($thirty_step[8]),
            'CAT' => trim($thirty_step[9]),
            'SUBCAT' => trim($thirty_step[10]),
            'PESO' => trim($thirty_step[11]),
            'IMAGEN' => trim($thirty_step[12]),
            'DESCRIPCION2' => html_entity_decode(str_replace('<textarea>', '', str_replace('</textarea>', '', str_replace("'", "", $thirty_step[13])))),
            'URLFAB' => trim($thirty_step[14]),
            'FTECNICA' => trim($thirty_step[15]),
            'PREQUERIDO' => trim($thirty_step[16]),
            'PSUGERIDO' => trim(str_replace("</tr>", "", $thirty_step[17])),
			'update' => $today,
            'IDMANUFACTURE' => str_replace('"', '', str_replace('>', '', $thirty_step[0])),
            'update' => $today
        );
        Db::getInstance()->update('linkstoreps', $updateData, '`IDa` = ' . trim($ida[0]['IDa']));
    } else {
